    ...
    return (
      <header className={`meta ${comment.author.badge ? ' has-badge' : ''}`}>
        ...
        <h3 className='name'>
          ...
          {
            comment.author.areaOfInterest && 
            <span style={{display: 'block', color: '#5873bf', fontWeight: '500', fontSize: '11px', marginTop: '3px'}}>Sector: {comment.author.areaOfInterest}</span>
          }
        </h3>
        {
          !!this.props.user &&
